...
当前版本：`0.6.0-demo4`。开局可选翡翠游侠、船骸鲨客或建筑师；电脑从其余两位随机选择。三位角色均使用骨骼动画，鲨客和建筑师新增逐件角色美术及技能立绘。
...
& '.\.tools\godot\Godot_v4.7.2-stable_win64_console.exe' --headless --path . --script tests/v4_acceptance_test.gd
```
...
- 点击角色专属技能按钮（“三连”“重击”“建造”）使用每局一次的行动：游侠三箭各造成 80% 普通伤害；鲨客单箭造成 150%；建筑师在己方区域点击放置 120×150 壁垒（约 1.5 个角色身高），占一回合，挡两次箭矢。鼠标悬停按钮可查看技能全称及说明。
...
翡翠游侠由 17 根骨骼驱动 16 个刚性美术拆件；鲨客和建筑师各由 17 根骨骼驱动 15 / 17 个透明拆件，动作由关键姿势实时插值，战斗中包含拉弓、放箭、受击及角色专属动作。数值集中在 `scripts/game_balance.gd`；电脑和玩家使用同样的技能次数，预测和实箭共享弹道与壁垒碰撞逻辑。建筑师使用次数仍可在该配置中调整，本版暂按每局一次实现。

所有骨骼动画只改变视觉子节点，不移动角色根节点，也不参与命中检测。Demo 4 可运行 `tests/v4_acceptance_test.gd` 检查角色拆件、动画及技能按钮；前版验收见 `验收记录_Demo3.md`。
